...
Task: Experiment with at write/read_csv() 

Like we did in class, I wil make a new data table of a subset of the gapminder data:
...
This is a grpah we made in lecture 8:
```{r}
graph1 <- gapminder %>% 
  filter(continent == "Americas") %>% 
...
```{r}
ggsave(here("Exercise5.png"), plot = graph2)
```

Here is the ![resulting file:](/Exercise5.png)
