...
1.1 Start by grouping by continent and using filter
```{r}
(gapminder2 <- 
  gapminder %>%
  group_by(continent) %>% 
...
```
1.2 Now, making a plot. I want to make grouped columns, how do I do this??
...
1.3 But now I can compare the minimum and maximum GDP for each continent in one plot (including data from all years). For most continents, there is a drastic difference between the max and min GDP. The mins are all pretty low, and for Asia, the max is very high.
